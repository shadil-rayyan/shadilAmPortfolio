...
export async function CodeCompassSection() {
  ...
  let codeData = defaultData;

  try {
    const [codeSetting] = await db
      .select()
      .from(settings)
      .where(eq(settings.key, "code_compass"))
      .limit(1);

    if (codeSetting?.value) {
      const parsed = JSON.parse(codeSetting.value);
    ...
    console.error("Failed to fetch CodeCompass settings:", error);
  }
